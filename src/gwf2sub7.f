...
C     ------------------------------------------------------------------
     ...
     1                      IDBIT, !JDH 08/16/2019
     2                      AC1,AC2,NDF,NNDF,ISBOCF,ISBOCU,
     3                      OCFLGS,OCLAY,ILSYS,NTSSUM,LN,LDN,NZ,RNB,
     4                      DH,DHP,DHC,DZ,HC,SCE,SCV,DCOM,
     5                      A1,A2,BB,SUB,DP,DVB
C
      DIMENSION IFL(13)
...
C     ------------------------------------------------------------------
      ALLOCATE (IIBSCB,ITMIN,NNDB,NDB,NMZ,NN,ND2,IDSAVE,IDBIT)
      ALLOCATE (AC1,AC2)
      ALLOCATE (NDF,NNDF)
...
C  JDH 08/16/2019 - ADDED IDBIT
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IDBIT,R,-IOUT,IN)
   ...
   55 FORMAT(' RESTART INFORMATION WILL NOT BE READ FOR DELAY',
     1 ' INTERBEDS')
      ENDIF
      IF(IDBIT.GT.0) WRITE(IOUT,56)
   56 FORMAT(' OPTIONAL DELAY BED ITERATION WILL BE PERFORMED')
C
...
C4------ADD APPROPRIATE TERMS TO RHS AND HCOF
  ...
  110  CONTINUE
      ...
      IF(NDF) THEN
       ...
       END IF
C
C5------INITIALIZE VARIABLES FOR DIRECT SOLUTION OF HEAD IN INTERBED
...
C6--------ADD CHANGES TO HEAD VALUES TO GET HEAD AT CURRENT ITERATION.
         ...
         END DO
C
C---------TERMINATE IF DELAY BED ITERATION NOT ALLOWED
C         THIS IF THE ORIGINAL SUB IMPLEMENTATION
         IF (IDBIT.LT.1) EXIT DBITER
C
C---------TERMINATE IF AC2 IS NOT ONE
         IF (AC2.NE.ONE) EXIT DBITER
C
C---------TERMINATE IF DHMAX IS LESS THAN SMALL CRITERIA
         IF (ABS(DHMAX).LT.SMALL) EXIT DBITER
C
C---------RESET DHMAX
         DHMAX = ZERO
...
C
C-------ORIGINAL APPROACH - LOADED ON RHS
      IF (IDBIT.LT.1) THEN
        STRGS=ZERO
        L1=LOC3
        L2=LOC3+NN-1
...
C
C-------REVISED APPROACH - LOADED ON HCOF AND RHS
      ELSE
C
...
C
      ...
      DEALLOCATE (GWFSUBDAT(IGRID)%IDBIT)
