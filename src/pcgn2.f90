    ...
    DO K=1,NLAY
       ...
       DO I=1,NROW
          ...
          DO J=1,NCOL
             ...
             NLN = NODE + NRC
             NLL = NODE - NRC
             ! ... PROJECT CONNECTION IDENITY LATERALLY
             IF (I.NE.NROW) THEN
                IF (IBOUND(NRN)/=0) THEN
                   CK_D(NRN)=CK_D(NODE)
             ...
             IF (J.NE.NCOL) THEN
                IF (CK_D(NCN)/=0) THEN
                   ! ... CHECK SIMILITUDE OF FORWARD CONNECTION IDENITY
                ...
                ELSEIF (IBOUND(NCN)/=0) THEN
                   ! ... PROJECT CONNECTION IDENITY FORWARD
                   CK_D(NCN)=CK_D(NODE)
...
510 FORMAT (/,1X,' THE DOMAIN INTEGRITY APPEARS TO BE COMPROMIZED', &
